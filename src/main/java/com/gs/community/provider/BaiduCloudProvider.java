...
public class BaiduCloudProvider {
    ...
    public ResultDTO getTextCensorResult(String text) {
        if (censorEnable == 0)
            return ResultDTO.resultOf(1, "未开启自动审核，自动通过");
        //初始化一个AipContentCenter
        ...
        if (response.has("error_code")) {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                if (msg == null)
                    msg = thisData.getString("msg").replace("百度", "");
                else
                    msg = msg + "以及" + thisData.getString("msg").replace("百度", "");
            }
